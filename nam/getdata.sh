...
tz=`cat nam.tz.txt | awk '{print $4}' | grep nam | grep 'conusnest.hiresf60' | grep -P 'grib2$' | cut -f2 -d'.' | sort | uniq | tail -n1`
...
startdata=`echo $date | cut -c5-12`
starttime=`echo $startdate $starthour`
fs=`cat nam.tz.txt | awk '{print $4}' | grep nam | grep "${tz}" | grep conusnest | grep -P 'grib2$'`
for f in `echo $fs`; do 
