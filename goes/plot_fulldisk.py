...
_, minlon, minlat, maxlon, maxlat, fsite, fi, fo = sys.argv
...
startdaynormal = datetime.datetime(int(start[1:5]),1,1) + datetime.timedelta(int(start[5:8]) - 1)
startdaynormal2 = startdaynormal.strftime('%a %d %b %Y')
enddate = end[1:5] + '-' + end[5:8] + '-' + ':'.join([end[8:10], end[10:12], end[12:14]])
enddaynormal = datetime.datetime(int(start[1:5]),1,1) + datetime.timedelta(int(start[5:8]) - 1)
enddaynormal2 = enddaynormal.strftime('%a %d %b %Y')
time = ':'.join([start[8:10], start[10:12], start[12:14]])
...
grid = remap(fi, extent, resolution, 'HDF5')
...
bmap.drawparallels(np.arange(-90.0, 90.0, 2.0), linewidth=0.25, color='white', labels=[True,False,False,True])
bmap.drawmeridians(np.arange(0.0, 360.0, 2.0), linewidth=0.25, color='white', labels=[True,False,False,True])
...
# Insert the legend
if band <= 6:
    cb = bmap.colorbar(location='right', size = '2%', pad = '0.5%', ticks=[20, 40, 60, 80])
    cb.ax.set_xticklabels(['20', '40', '60', '80'])
else:
    cb = bmap.colorbar(location='right', size = '2%', pad = '0.5%'
    )
...
if (fsite != '-'):
    with open(fsite) as site:
        ...
        for name, lat, lon in pat.findall(cont):
            ...
            bmap.plot(lons, lats, '+', markersize=6, color='r')

# Convert time zone
from_zone = dateutil.tz.tzutc()
to_zone = dateutil.tz.tzlocal()
utctime = datetime.datetime.strptime(enddaynormal2 + ' ' + time, '%a %d %b %Y %H:%M:%S')
utctime = utctime.replace(tzinfo=from_zone)
localtime = utctime.astimezone(to_zone).strftime('%a %m/%d %H:%M:%S')
